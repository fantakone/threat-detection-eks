...
The collectors use `boto3` which automatically picks up credentials — **no credentials are ever stored in the code**.
...
**Option 3 — IAM Role (for EC2 / EKS)**
If running on an EC2 instance or inside a Kubernetes pod with an attached IAM role, boto3 picks up credentials automatically.

> **Note:** Credentials are only needed for `cloudwatch` and `prometheus` source modes. The `simulator` and `local` modes work without any AWS credentials.
